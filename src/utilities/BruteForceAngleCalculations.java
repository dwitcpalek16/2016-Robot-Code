	...
	public static void writing(String line, boolean newLine, int extension)
	{
		try
		{
			...
			PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(
					"C:/Users/Derek/Desktop/AngleCalculations/angleCalculations" + extension + "InchExtension" + ".csv",
					true)));
		...
		for (double x2 = -10; x2 <= 15; x2 += 1)
		{
			for (double H2 = 0; H2 <= (L + l + h); H2 += 5)
			{
				for (double theta1 = 0; theta1 < 180; theta1 += 0.1)
				{
					...
					{
						height = (h + l * Math.sin(theta1 * Math.PI / 180)
								+ L * (Math.sin((theta2 - theta1) * Math.PI / 180)));
						extension = (L * Math.cos(((theta2 - theta1) * Math.PI / 180))
								- (x1 + l * Math.cos(theta1 * Math.PI / 180)));

						if (extension <= (x2 + 0.05) && extension >= (x2 - 0.05) && height <= (H2 + 0.05)
								&& height >= (H2 - 0.05))
						{
							double[] angleSetpoints =
							{ height, extension, theta1, theta2 };
				...
				{
					if (((Math.abs(goodValues.elementAt(i)[0] - H))
							+ (Math.abs(goodValues.elementAt(i)[1] - x2))) < ((Math.abs(temp[0] - H))
									+ (Math.abs(temp[1] - x2))))
					{
						temp = goodValues.elementAt(i);
					}
				}
				System.out.println(H2 + ": height: " + temp[0] + "-----extension: " + temp[1] + "-----theta 1: "
						+ temp[2] + "-----theta 2: " + temp[3]);
